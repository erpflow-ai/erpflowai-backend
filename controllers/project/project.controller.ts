...
export const updateProjects = (req: Request, res: Response) => {
    ...
    fs.readFile(filePath, 'utf8', (err, data) => {
        ...
        const newClub = {
            id: workorderDetails.clubs.length > 0 ? workorderDetails.clubs[workorderDetails.clubs.length - 1].id + 1 : 1,
            ...req.body
        };
        ...
        fs.writeFile(filePath, JSON.stringify(workorderDetails, null, 2), (writeErr) => {
            if (writeErr) {
                return res.status(500).json({ message: 'Error writing file' });
            }

            res.status(200).json({ message: 'Club added successfully', club: newClub });
        });
    });
};
export const getAllProjects = (req: Request, res: Response) => {
    const filePath = path.join(__dirname, '../../utils/data/projectDetails.json');
